...
- **Time model: 1 in-game day = 1 run.** All "per day" passive effects (Knowledge tick, building production) fire on completing a run — win or die. Nothing advances in real time. [Kills the idle-game incentive to close the game and wait; runs stay the heartbeat of everything.] **Amended 2026-07-10 (human decision — magnitude, not trigger):** the day COUNTER still advances 1 per run and the tick still fires once at run end (win or die; forfeit ticks nothing), but the tick's MAGNITUDE now scales with rooms cleared — `scale = rooms_cleared × PER_ROOM_TICK` (0.1 placeholder, `TownCore`; a "nominal day" = 10 rooms). Rationale: a flat tick paid a room-1 suicide the same passive production as a 40-room clear, incentivizing suicide runs; the scale removes that while fiction and dialogue gates keep riding the 1-day-per-run counter.
