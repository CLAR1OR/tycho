...
const SPEED := 17.0      # FEEL: arrow travel speed (m/s)
...
func setup(dir: Vector3, damage: int) -> void:
	_damage = damage
	var d := dir
	if d.length() < 0.001:
		d = Vector3.FORWARD
...
func _on_body_entered(body: Node) -> void:
	if body.has_method("take_damage"):
		...
		CombatFX.damage_number(get_parent(), body.global_position + Vector3.UP * 1.6, _damage, Color(1.0, 0.5, 0.5))
