...
**Next agent chunks (agreed order):** boss #1 build (direction A "The Den-Warden" PICKED 2026-07-10 — `design/bosses/floor-1-boss.md`) → dialogue volume (define remaining arc beats first, add a `max_floor` counter) → remaining tech nodes (blocked on the content-gate verdict; 8 are now forward-referenced by building gates — see `design/town-economy.md` § tech payoff).
