...
**Content vs budget (the real gap — the engine is done, the game is ~15% authored):** tech nodes 2/14 · echoes ~25/~50 · enemies 5/12 · bosses 1 placeholder/5 · buildings 9/9 data (+2 shops; models 0/9) · attunements 7/~7 (placeholder) · dialogue ~52/~370 pieces · achievements 0/~25 · spine ~9/~22 scenes (Phases C/D mostly unauthored). Counts: `design/content-budget.md`.

**Validation baseline:** **255 unit test cases** + **307 smoke ok-checks** (±1 known door-sigil nondeterminism) + editor pass clean. Commands + rules (throwaway slot 99, one smoke per invocation, profile.json is GLOBAL — the human's real profile): `design/godot-conventions.md` § Testing.
...
**Next actions (HUMAN):** (0) pick boss #1's direction (`design/bosses/floor-1-boss.md` §3 — A beast / B terrain / C doppelgänger; A recommended); (1) content-gate verdict — research Arithmetic then Masonry at Sophia's desk, judge *delight vs. homework*; (2) asset-pipeline gate — one rigged `.glb` per `assets/README.md`; (3) curate dialogue — `design/dialogue/drafts-review-2026-07-06.md` first (§6 has placeholder E2 lines), then `drafts-review-2026-07-04.md`; (4) ongoing dials — feel numbers, strata palettes + the 5 legibility passes, audio mix; Kenney packs + Suno-vs-Udio + the dream-motif instrument (`design/audio.md`).

**Next agent chunks (agreed order):** boss #1 build (design doc DRAFTED 2026-07-10 — `design/bosses/floor-1-boss.md` carries the all-boss grammar + 3 directions; **blocked on the human direction pick**) → dialogue volume (define remaining arc beats first, add a `max_floor` counter) → remaining tech nodes (blocked on the content-gate verdict; 8 are now forward-referenced by building gates — see `design/town-economy.md` § tech payoff).
