...
**Next agent chunks (agreed order):** boss #1 design doc (`design/bosses/`, unblocked by the feel gate) → dialogue volume (define remaining arc beats first, add a `max_floor` counter) → remaining tech nodes (blocked on the content-gate verdict; 8 are now forward-referenced by building gates — see `design/town-economy.md` § tech payoff).
