...
- **UI = "Ember" everywhere (HUD built 2026-08-10; menu base built 2026-08-13):** the language is **no panels** — everything floats on the world on hairlines, thin rings and negative space, gold reserved for state; anchors `assets_src/anchors/in-run-hud-reference.png` + `weapon-menu-reference.png`. `EmberHud` (`src/core/ember_hud.gd`) owns the palette, both primitive sets (HUD + menu: scrim/row-box/dashed-frame/section/pips/prompt/flourish) and **22 code-drawn glyphs** — no icon assets, so the whole UI is independent of the asset gate. `RunHud` is the one migrated surface. **2026-08-13 (human: "I want all my ingame HUD and menus in the new style"):** Slate is now **legacy** — new pure `EmberMenuCore` (4-column + centred-column layout grammar, stat deltas, cost rows, pip tracks), new `EmberTheme` (the Control-tree half; Panel = **transparent**, the inversion vs Slate), a **fourth font** — Alegreya Sans, the interface voice, closing the deliberate gap — and `tools/render_menu.tscn` (specimen-screen probe). **Tier A migrated same day** — `TownHud`, `AchievementToast`, `EchoOfferPanel` (all restyles, API byte-identical) + `PauseMenu`, the first `EmberTheme` screen. **Ember 5 / Slate 11; Slate is deleted when the last one moves** (`design/ui-hud.md` § "Migrating to Ember"; Tier B = the six town facility screens). Probes: `tools/render_hud.tscn` (4 overlay states) + `tools/render_menu.tscn` (3). **HUMAN:** every value is a placeholder — judge the probes + F5 (dials: `design/feel-tuning.md` §§ Ember HUD / Ember menus / Ember Tier A).
...
**Next actions (HUMAN):** (0) **judge Ember** — `tools/render_menu.tscn` beside `assets_src/anchors/weapon-menu-reference.png`, `tools/render_hud.tscn` for the four overlays, then F5 (town + a run) over painted-lite; dial `EmberHud` + `EmberMenuCore` **before** Tier B, so eleven more screens inherit dialed values instead of placeholders; (1) content-gate verdict — research Arithmetic then Masonry at Sophia's desk, judge *delight vs. homework*; (2) **asset pipeline — start at `design/asset-pipeline.md` Stage 1 (materials, Recraft-only month ~$10, no 3D tooling), then Stage 0 (the gate: one rigged `.glb`, needs a paid Tripo month — free-tier outputs are public/non-commercial)**; (3) curate dialogue — `design/dialogue/drafts-review-2026-07-06.md` first (§6 has placeholder E2 lines), then `drafts-review-2026-07-04.md`, then `drafts-review-2026-07-10.md` (the whole spine + the boss rename); (4) ongoing dials — feel numbers, strata palettes + the 5 legibility passes, audio mix; Kenney packs + Suno-vs-Udio + the dream-motif instrument (`design/audio.md`).

**Next agent chunks (agreed order):** **UI migration to Ember** — Step 1 (menu base) + Tier A both DONE 2026-08-13; **next = Tier B** (`ForgePanel`, `EtchingsPanel`+`AttunementsPage`, `BuildPanel`, `SurveyPanel`, `MarketPanel` — the anchor's own catalogue archetype; Market + Attunements also need their missing `ui-hud.md` sections written), then Tier C (tech/slot-select/settings/achievements/dialogue), Tier D (delete Slate) — tiers + rules in `design/ui-hud.md` § "Migrating to Ember". → remaining tech nodes (blocked on the content-gate verdict; 8 are forward-referenced by building gates — see `design/town-economy.md` § tech payoff; 3 dialogue pieces also sit dormant on them). Other unblocked candidates: enemies 5→12, echoes ~25→~50, bosses 2–5.
