...
> **Implementation status (BUILT 2026-07-10, direction A):** the Den-Warden is live on floor 1, replacing the stats-pumped dummy there. **Data:** `data/bosses/den-warden.json` (new DataLoader domain `bosses`; final shape in `architecture-schemas.md` §11 — `{id, name, floor, hp, contact_damage, reconfigure_s, phases:[{threshold, loop[]}], moves:{id:{kind,…}}, arena_vents:[[x,z]]}`; phase 1 loop lunge/swipe/circle at 1.0, phase 2 burrow/erupt/vent_call at 0.5). **Code:** pure `src/combat/boss_core.gd` (`BossCore.phase_for/next_move/should_reconfigure/validate/def_for_floor`, 9 unit suites in `tests/core/boss_core_test.gd`) + `src/combat/enemy_boss.gd` (`EnemyBoss extends EnemyDummy` on `scenes/combat/enemy_boss.tscn` — a def-less instance falls through to the plain dummy machine, so **floors 2–5 keep the placeholder boss + escorts unchanged**); `combat_room.gd` picks the current floor's def, spawns a clean no-escort arena with the def's vent plates as REAL dormant `vent-plate` hazards (new `Hazard.set_dormant`/`fire_once` command API — only `vent_call` fires them); the RunHud bar shows the def's `name`. Reconfiguration beat + burrow are invulnerable, resolve on their own, and cleanly ignore damage (smoke-proven; a one-shot from full HP skipping phase 2 is legal). Bounty / heal valve / echo pick / `boss_killed` (now with id `den-warden` on floor 1) unchanged. **All numbers placeholders** — dial board: `feel-tuning.md` § Boss — Den-Warden (data numbers + the `move_beat_s`/`reconfigure_pulse` exports). **Rule-7 dialogue hooks DRAFTED (2026-07-10, dialogue-volume pass):** first-kill (`sophia-burrower-kill`), death-streak debrief (`tilly-burrower-death`), the floor-1 shard (`sophia-first-floor-shard`), the stratum-1 report — none uses the placeholder name (a pool-wide unit lint enforces it), so the human rename orphans nothing; curation pending (`design/dialogue/drafts-review-2026-07-10.md`). **Deferred:** the real name (human, with curation), the real model/silhouette + nest/bones prop note (asset-pipeline gate), the human feel pass on every tell/beat, the same-roar-every-time hindsight tell (needs audio), any reconfiguration sting (open question below).
>
