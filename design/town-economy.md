...
>
> **Implementation status:** NOT built beyond what already existed (Sophia's Study, Farm, Quarry, Town Walls ×3 levels + the Well-Fed day tick, `food-upkeep.md`). Everything else here is the next town chunk.
...
- **Gold saturation can't recur.** The economy sim's headline red flag (94k idle gold by run 40, no sink once buildings max — `economy-sim.md`) is answered three ways: the Market's repeatable exchange, the Cathedral's huge staged costs, and every later age reopening ~2 levels × 9 buildings of sinks.
...
2. **Exchange (the repeatable soft sink):** buy at unfavorable rates — **5 gold → 1 Stone**, **2 gold → 1 Food**. Deliberately worse than producing it; late-game gold finally has somewhere to go, and it directly relieves the sim's real bottleneck (Stone).
...
- **Stage costs meant to bite:** 400g + 60 Stone → 900g + 150 Stone → 2,000g + 300 Stone. Sized against the sim's 94k-gold-by-run-40 curve; the human dials.
...
- Dust two-sink saturation (`economy-sim.md`) is a *combat*-economy problem — explicitly NOT addressed here (IC-14 separation); needs its own pass.
