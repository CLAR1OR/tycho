...
>
> **Deferred:** the Observatory's `puzzle-hint` capability ships as data with NO consumer (hints currently cycle freely in the puzzle UI) and its next-tech preview is unbuilt; exchange + caravan are not in the economy sim (policy note in `economy-sim.md`); new-building silhouettes fall back to monograms (placeholder-first). *(The Cathedral completion achievement is no longer deferred — it landed with the achievements system 2026-07-11 as `data/achievements/cathedral-complete.json`.)*
>
...
- **Completion bonus (placeholder):** shard turn-in value +1 (`SHARD_KNOWLEDGE_VALUE` 5→6) + an achievement (**live 2026-07-11:** `data/achievements/cathedral-complete.json` — `building_built` where `building_id=cathedral`, `level >= 3`). Prestige first; the bonus is themed, not build-defining.
