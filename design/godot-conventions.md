...
- **HUD layout probe:** `godot --path . tools/render_hud.tscn` — renders the in-run HUD (`RunHud`, the Ember design) into a **1280×720 SubViewport** (the project's base viewport, never whatever size the window manager hands the probe) over a dark field with bright test patches, and writes `user://hud_render_{combat,low-hp-cleared}.png`. It fakes the room state the HUD would otherwise need a real run to reach — mid-wave with peril, and low-HP/cleared — and stands up a real `Player` so the ability dial shows real glyphs and cooldown arcs. **NOT `--headless`** (it reads the viewport texture back). It is a *layout* probe, not a screenshot of the game: it catches collisions, off-screen elements and illegible text before they cost an F5 run; the real judgment is still in-engine. Add a state by adding an `await _shot(...)` block.
- **Menu layout probe:** `godot --path . tools/render_menu.tscn` — renders a **specimen** menu screen (fake weapon-menu content matching `assets_src/anchors/weapon-menu-reference.png`) into a 1280×720 SubViewport over a mid-tone field, in both Ember menu layouts, and writes `user://menu_render_{catalogue,column}.png`. It exercises every menu primitive at once so the shared vocabulary can be judged **before** the fifteen Slate screens migrate onto it (`design/ui-hud.md` § "Ember menu vocabulary"). Touches no game state, no `data/`, no save. **NOT `--headless`** (it reads the viewport texture back). The HUD's sibling: `render_hud.tscn`.
