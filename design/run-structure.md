...
> **First implementation built 2026-07-05** (pure `DoorCore` + `data/floors/` + `data/resources/resonance-dust.json`; door portals, Wellspring, boss/echo valves in combat_room.gd + game.gd; the every-combat-room echo offer is retired — echoes now come from echo doors + one post-boss guarantee). All numbers below are the placeholders that shipped (in `data/floors/*.json` + `DoorCore` constants). **Deferred (designed here, not built):** Cartography **foresight**, **3-door** offers, per-floor **heal caps**, first-encounter door **tooltips** (with the painterly pass). (**Recovery attunement is now BUILT** — 2026-07-10, with the Passive Attunements system: L1/2/3 = 4/7/10% of missing HP healed on room clear; see `design/etchings.md` implementation-status block.) **The 3 healing echoes are now BUILT** — 2026-07-10, with the echo-pool expansion (8→~25): **Mender's Rhythm** (`menders-rhythm`, heal 4% of missing on kill), **Deep Repair** (`deep-repair`, +50% healing received — multiplies EVERY heal uniformly), **Salvage** (`salvage`, heal 6% of missing on ore/dust pickup); values are placeholders in `data/echoes/*.json`. New player hooks: `heal_on_kill_pct` (consumed in `combat_room._on_enemy_died`), `heal_received_mult` (folded inside `player.heal`, so Wellspring/valve/Recovery/echoes all scale), `heal_on_pickup_pct` (consumed via `combat_room._on_resource_changed` on the ore/dust `resource_changed`). All heals stay % of MISSING HP.
