...
> **The migration is COMPLETE.** `SlateHud` and `SlateTheme` were deleted on 2026-08-14 (Tier D) — 15 surfaces and 7 helper draw classes moved across three tiers in two days, and **Ember is the only UI language in the game**. Anything in the per-screen sections below that still reads "Slate" is a historical record of how that screen was built, not how it is drawn now.
...
| **D — retire Slate ✅ DONE 2026-08-14** | delete `SlateHud` + `SlateTheme` | a pure deletion — both files were already orphaned. **There is one UI language in the game again** |
...
*(Historical: until 2026-08-13 both HUDs shared a `SlateHud` base instead. It was deleted with the rest of Slate on 2026-08-14.)*
...
## Panels & the pause menu (2026-07-07 — **historical: this was the Slate `Theme` pass; Slate was deleted 2026-08-14**)

> Human directive at the time: "continue with the pause/panel restyle to slate. pause menu should be fullscreen."

**What this pass established, and what carried over.** Six Control-*tree* UIs (`PauseMenu`, `ForgePanel`, `TechPanel`, `EtchingsPanel`, `DialoguePanel`, `EchoOfferPanel`) were plain default-gray. They do not draw in `_draw` like the HUDs — they are node trees of Labels/Buttons/Containers — so they took a shared **Godot `Theme`** rather than a draw base. **That split is still the architecture:** a screen laid out at fixed positions is an `EmberHud` subclass; a screen whose content is a variable number of rows that must scroll or reflow takes `EmberTheme`. See § "Ember menu vocabulary" for the live version.

**Two lessons from that pass are still load-bearing:**
- **A theme type variation may not shadow a built-in Godot class name** — `MenuButton` is a real class, so that variation silently never registered and nothing errored (found 2026-07-08, after a day). This is why `tests/core/ember_theme_test.gd` exists and asserts every Ember variation actually landed.
- **Applying a theme made the panels SIMPLER**, because `theme_type_variation` replaced ad-hoc `add_theme_font_size_override` / `modulate` fiddling per node. The same held true of the Ember migration.

**The pause menu became FULLSCREEN here** and has stayed so: `PauseMenu` changed base from `PanelContainer` to `Control` — a backdrop over the whole screen with a centred column (Cinzel `Paused` title, the Resume / Settings / Achievements / Forfeit / Quit buttons at min width 320, and the gate-refusal status line under the title). **Migrated to Ember 2026-08-13**, still a Control tree on purpose (real Buttons need hover/focus/disabled/click, which containers already do correctly): `theme = EmberTheme.get_theme()`, variations `EmberTitle` / `EmberDim` / `EmberAction`, the backdrop reading the shared `EmberHud.COL_SCRIM` (opaque since 2026-08-14), and **Resume alone** wearing the gold `EmberAction` frame — gold is state, so five gold buttons would say nothing. Judge with `tools/render_menu.tscn`'s `pause` state. **CanvasLayer geometry quirk** (the RunHud bug): anchors set in this Control's own `_ready` under game.gd's `$HUD` CanvasLayer get no layout pass, so `size` is synced to `get_viewport_rect().size` in `open()` AND each frame in `_process` while visible (it is PROCESS_MODE_ALWAYS). The smoke opens it and asserts the root spans the viewport. All flow (tree pause/unpause, the guards, inert-at-slot-select, the Hades gate) is untouched.

**Still un-themed by design:** the F1 tuning panel and the F2 cheat panel (debug tools), and the arch puzzle (`puzzle_arch.gd` draws its own diagram).
...
**Colour source:** the stone/food/knowledge resource colours were lifted UP from `TownHud` to the shared base (next to gold/ore/dust/shards) so the chart reads the knowledge colour off the ONE dial source.
...
- **The skill menu** (right-side dock, opens on click) — sigil + name (Cinzel) + key chip (`RMB`/`Q`/`R`/`SPC`); a chips row (principle in dust-cyan + `%ds cooldown` dim mono); the description (Garamond, from data); the level track; the action button.
...
`SlotSelect` (`src/core/slot_select.gd`) is a fullscreen `Control` (base was `PanelContainer`), Ember-themed since 2026-08-14 (`EmberTheme.get_theme()`), `PROCESS_MODE_ALWAYS`, in group `slot_select`, with the CanvasLayer viewport-size sync in `_process` (the geometry quirk the other rebuilt screens handle). It only browses + deletes; it emits `slot_chosen(slot)` and game.gd does the loading + checkpoint-resume routing (unchanged). Meta strings are the pure **`SlotSelectCore`** (`src/core/slot_select_core.gd`, unit-tested).
...
**HUMAN dial placeholders:** the sky gradient stops + star/constellation/ridge geometry + title size/spacing/glow (all consts atop `slot_select_sky.gd`), the plaque sizes / chip styles / delete-button styling (consts atop `slot_select.gd`), and ALL flavor copy (`choose a saga`, `New saga`, `an unwritten sky`). The shared palette/fonts live in `EmberHud`. Dial like FEEL numbers.
...
- **Carry readout** (top-right, bare — no box) — one row per resource in THIS building's level costs (typically gold + stone): a dim label + a BIG mono number in the resource's shared colour (the E1/F2 Dust/Ore readout pattern). **Since Tier B this is the shared `EmberHud._resource_readout`** — glyph + number, right-aligned, the same mark on every screen.
...
**SettingsPanel** (`src/core/settings_panel.gd`, `class_name SettingsPanel extends EmberHud`, group `settings_panel`) — a fullscreen `Control`, `PROCESS_MODE_ALWAYS`, mouse_filter STOP, over a near-opaque backdrop (`#0c0b10 @ 0.88`, a LOCAL placeholder const reused from the pause menu). Everything but the backdrop is drawn in `_draw`; mouse hit-testing in `_gui_input`; ESC / arrows in `_input`. Spawned on demand onto `game.gd`'s `$HUD` via `game.open_settings()`; `queue_free` on close. `size` syncs to the viewport in `_process` (the CanvasLayer-under-`_ready` quirk — NOT set synchronously in `open()`, which runs right after `_ready` and would warn).

**Components** (all sizes/colours/copy HUMAN-placeholder consts atop the file; palette/fonts read off `EmberHud` only, plus the two locals `COL_HOVER_BRIGHT #bff2ff` and `COL_NOTCH_UNLIT`):
...
**HUMAN placeholders:** the backdrop/hover/notch colours + all sizes (consts atop `settings_panel.gd`), `SettingsCore.STEP` + the row labels, and ALL copy (title/subtitle/section heads/chip labels/the ASSIST name+note/footer). Shared palette/fonts live in `EmberHud`.

## Achievements — page + unlock toast (built 2026-07-11; **page migrated to Ember 2026-08-14**)

> Built WITH the achievements system (`architecture-schemas.md` §5) — no claude.ai/design round yet; both pieces were first-pass Slate placeholders the human restyles freely.
