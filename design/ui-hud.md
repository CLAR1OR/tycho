...
2. **Resource strip — top-right.** One slate panel row of seven columns in order gold / stone / food / knowledge / shards / ore / dust (Ledger ids `gold`, `stone`, `food`, `knowledge`, `knowledge-shards`, `resonance-ore`, `resonance-dust`): value (mono, per-resource colour) then a small dim label. Amounts poll `Ledger.get_amount` per frame. Three new resource colours (stone `#b5ada0`, food `#a4d97a`, knowledge `#9fdcff`) are TownHud consts; gold/shards/ore/dust reuse the shared pickup colours.
