...
| # | Beat | Gate |
| --- | --- | --- |
| C1 | **First full clear.** Final boss falls; the **codex artifact** is revealed in the chamber — first shard slots in, the assembly begins. [cutscene] *(the artifact pedestal + dissolve mechanic is BUILT 2026-07-07: after the final boss's valve, a codex-artifact pedestal rises as the only way out; walking into it dissolves Tycho and returns him to town victorious, slotting the run's shard. The C1 CUTSCENE itself is still unauthored — the mechanic delivers the pedestal, not the scripted reveal.)* | first full clear |
| C2 | **The question nobody asks.** Sophia, quietly: shards teach *real things that check out* — who writes knowledge into stone, and why is it *true*? First rational-fiction itch at the artefact's nature. | C1 + next town visit |
...
| D5 | **Cliffhanger close.** Tilly promoted to organize defense; Sophia looks west; "armies, not just a sword." **End of v1.** | D4 |
...
- counters: `runs >= N`, `deaths >= N` (combat deaths only), `dissolves >= N` (full-clear returns via the codex artifact — 2026-07-07), `boss_kills >= N` (cumulative), `full_clears >= N`, `codex_shards >= N`
