...
| A3 | **First run home → the town learns.** Back at home; Tycho takes his etchings to Sophia and walks her to the portal — which **refuses her** (keyed to him). Establishes: only Tycho can enter, Sophia analyzes. *(authored 2026-07-06 from the human's script. **Twin pair (2026-07-07):** `a3-first-return` — generic, force-play cutscene, gate `runs >= 1`, priority 104 — fires on the FIRST run's town return regardless of death; `a3-first-death` — the death-specific monologue, gate `deaths >= 1`, priority 105 — wins the slot when the first run ended in death. Both set `a3`; shared-flag suppression keeps only one from playing. A first-meeting reaction for Mara, `a-mara-meets`, gates on `flag(a3)`.)* | first run home (`runs >= 1`; the death variant additionally `deaths >= 1`) |
...
| E2 | **The artifact waits.** The final shard slots in — the artifact assembles fully, turns once, hums… and *stops*. It is complete, and it is **waiting**. Sophia: "It's finished. And it's — listening? For what?" Thomas (if seen after): "A finished question is still a question." Explicit, honest, in-fiction "not yet" — solving it is Act II+'s door (per IC-6: v1's payoff is the grown mystery). Written **order-safe**: no emperor references unless `d5` is set (it can fire pre-climax for a completionist rusher). [cutscene] | codex shards = max (all slots filled) |
...
- **A3 win-first-run edge (raised 2026-07-06). RESOLVED 2026-07-07 (implemented):** went the **twin-copy** route (same shape as B3). `a3-first-return` (force-play cutscene, gate `runs >= 1`, priority **104**, `sets_flag: "a3"`) is the generic opener that fires on the FIRST run's town return regardless of death; `a3-first-death` (gate `deaths >= 1`, priority **105**, same `sets_flag`) is the death-specific variant. After a death both gates hold (a death still ticks `runs`), so priority 105 wins and the death version plays; after a deathless first run only the generic is eligible. Either sets `a3`, and `DialogueCore`'s shared-flag suppression silences the other forever. The whole a3 chain (Mara's first-meeting, Thomas/B2 → etchings) now fires for winners and losers alike.
