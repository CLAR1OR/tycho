...
| A3 | **First death → the town learns.** Wake at home; Tycho works out he really died and came back (no dream), takes his etchings to Sophia, and walks her to the portal — which **refuses her** (keyed to him). Establishes: death is survivable, Sophia analyzes, only Tycho can enter. *(authored 2026-07-06 from the human's script: `a3-first-death`, force-play cutscene, sets `a3`. A first-meeting reaction for Mara, `a-mara-meets`, gates on `flag(a3)`.)* | first death (`deaths >= 1`) |
...
- **A3 win-first-run edge (raised 2026-07-06).** `a3-first-death` gates on `deaths >= 1`, and the a3 chain (Mara's first-meeting, Thomas/B2 → the etchings unlock) hangs off it. A player who **never dies on run 1** never trips the gate, so the first-death cutscene and everything downstream stall (including etchings). Options: an `OR runs >= N` fallback (same shape as B3's), or fire A3 after A2 regardless of death. Human call — not solved.
