...
- **Buses:** `Master → Music / SFX / UI` (+ `SFX/Combat` child if ducking is ever wanted). Volumes are **profile settings** (schemas §1) — **the settings screen is BUILT (SET1, 2026-07-09, `design/ui-hud.md` → "Settings — The quiet page")**: its three volume sliders live-apply to the buses via the (now public) `Music.apply_audio_settings()`; bus setup is a one-time `default_bus_layout.tres`.
