...
| IC-7 | **1 in-game day = 1 run.** All "per day" passives fire on run completion (win or die). Nothing advances in real time. *Amended 2026-07-10 (human): the trigger and the day counter are unchanged, but the tick's MAGNITUDE scales with rooms cleared (`TownCore.run_tick_scale`, PER_ROOM_TICK 0.1 placeholder, nominal day = 10 rooms) — kills the suicide-run incentive of a flat tick.* | Kills idle-game waiting; runs stay the heartbeat. | Production/research numbers. |
...
| IC-10 | **Graceful paths in both directions:** assist mode ("Reinforcement Protocol", nanobot-framed, capped stacking damage resist) for combat; Sophia auto-solve for puzzles. No content hard-gated behind either skill. | Inclusivity for the two audiences; diegetically the runs *are* learning trials. | Stack caps, auto-solve pacing. |
...
- **Run end (win OR die):** advances the **day tick** → town production + research fire once (tick magnitude room-scaled since 2026-07-10: `rooms_cleared × 0.1`, so a full ~40-room clear ≈ 4 nominal days and a room-1 death ≈ nothing; the day counter still advances exactly 1); new dialogue becomes eligible; everything collected is kept.
...
- **Resolution:** **5 floors × 6–10 rooms**, RNG-ordered from **30 shared combat layouts + 5 boss arenas + 3 reprieve layouts** *(**BUILT 2026-07-12** — `data/layouts/` + pure seeded `LayoutCore` [per-floor no-repeat pick, flood-fill path validation, footprint keep-outs]; placeholder arrangements, human dial pass pending; entry/exit rooms are not separate layouts in v1)*. One geometry kit; each floor is a **stratum** (IC-5 as revised 2026-07-03, spec `dungeon-strata.md`): a data-driven environment profile (palette/fog/light) + **one signature hazard** + 2–4 props. Floors differ by **enemy mix + stratum profile + signature hazard**, never by biome art. Hazards are scripted (timer + volume + telegraph), dual-use (hurt enemies too), density-tuned per floor. Boss at the end of every floor.
...
- **Content hooks:** layouts (`data/layouts/` — built 2026-07-12), room-type weights per floor, floor profiles (`data/floors/` — now incl. `door_weights` + `peril_chance`), hazards (`data/hazards/`), 6 door sigils + Wellspring prop.
...
- **State:** **12 base enemy types** target (~4–5 per floor with overlap) — **5 built so far** (all placeholder primitives + FEEL stats): **Brute** (armored melee), **Skirmisher** (fast squishy melee), **Archer** (ranged kiter), **Slammer** (2026-07-06 — lumbering AoE: paints a growing ground-circle telegraph, then a dodgeable slam; armored), **Charger** (2026-07-06 — line-telegraph dash-charge; squishy, stunned + vulnerable after, longer if it hits a wall). **elites = a modifier system on base types** (no new art) — **first concrete stub built 2026-07-05:** a **peril door** (`run-structure.md`) spawns an elite wave, runtime stat mults (placeholder `hp ×1.5 / dmg ×1.25`, applied at spawn over the exports, never a `.tscn` edit) for a doubled reward; the full modifier roster is later; **5 bosses, one per floor**, floor-5 = final boss (drops Codex Shards). Difficulty tiers reuse all 5 with new patterns/modifiers.
- **Resolution:** standard real-time AI (telegraphed attacks, tells); boss = arena + multi-phase patterns. **Combat rooms run 2–3 sequential waves** (2026-07-06, Hades-style): clear the wave → short beat + a spawn telegraph → the next wave; the room clears (doors/echo/caches fire) only after the last. Wave count/sizes/type-mix are the pure seeded `WaveCore` (placeholders); peril mults apply to every wave. Boss rooms stay a single staged fight.
...
- **Trigger:** unlocked at B3; player selects an active node; research accrues on the day tick + invested Knowledge. **Investing spends Knowledge ONLY** — Knowledge Shards (boss loot) are the resource the player brings back and **turns in to Sophia at the desk** for Knowledge (all held shards convert at `SHARD_KNOWLEDGE_VALUE` = 5 apiece; a "turn in shards" action, 2026-07-06). Diegetic: you bring her what you found; she extracts what it teaches.
- **State:** `tech.researched[]`, `tech.in_progress {id: knowledge}`, `tech.auto_solve_counters`, `tech.quiz_locked {id: true}`. Each node: `age`, `tier` (key/support), `cost_knowledge`, `prereqs`, **typed `unlocks[]`** (building/resource/capability now; unit/strategy-building/space-tech later — do NOT hardcode unlock kinds), `puzzle`, `auto_solve_after_runs`, `thinking_tool` (🧠).
- **Resolution:** accrue Knowledge into the active node → at threshold, **read explanation → solve puzzle/quiz → unlock fires** (typed dispatch applies the unlock). **Quiz gate (2026-07-06):** a WRONG quiz answer **locks that node's quiz until one more run passes** (`quiz_locked`, cleared on `run_ended`) — the interactive puzzle is exempt (its staged failures are the teaching). Sophia **auto-solves over ~5 runs** for players who skip or keep failing puzzles (reward thinking, don't hard-gate — a wrong answer delays, never blocks). Each first-researched tech of an age **advances the town to that age** (visible skin change).
- **Player decision:** which node to pursue; turn in Knowledge Shards now vs. save; solve vs. auto-solve.
...
- **Trigger:** town visit (between runs); the **day tick fires once per run-end** (TownTick reads buildings → writes ledger → emits events); its magnitude is room-scaled (2026-07-10, IC-7 amendment) — production AND Food upkeep both multiply by `rooms_cleared × PER_ROOM_TICK` before the Well-Fed evaluation. HUD projections stay per-NOMINAL-day (scale 1.0).
- **State:** Town as a **data object** (`{id, name, age, buildings[{id, level}], map_pos:null}`) — instantiated once in v1, N times in Act II. Buildings: category (production/research/infrastructure/shop/great-work), `unlocked_by`, **age-banded levels** (Age I = L1–3; each later age +2, cap L11; band openers tech-gated per level and may transform/rename the building — `town-economy.md`, designed 2026-07-10). Each level = cost + typed `effects[]` + `visual`, REPLACE semantics.
- **Resolution:** build/upgrade at Herzog (Gold + the age's Material — Stone in Age I). **9 buildable + 2 shops** (Timber/Woodcutter/Cart cut; University = the Library's Renaissance band; the Cathedral Great Work + the Market are the gold sinks). Each level changes the **visual** — the town visibly grows. Production resolves on the day tick.
- **Player decision:** what to build/upgrade with limited Gold + materials; research vs. production vs. infrastructure; whether to feed gold into the Market exchange or save toward the Cathedral.
- **Content hooks:** `data/buildings/`; effect kinds (`produce|knowledge|multiplier|capability|upkeep`; strategy kinds `defense|summon-capacity` extend later); roster/bands/sink specs in `town-economy.md`.
...
- *(2026-07-10: first drop-rate/cost rebalance pass landed, iterated against `tools/economy_sim.gd` — weapon flat costs, etching/attunement Dust costs, door-cache amounts, gold/ore drops, building L2/L3 gold all retuned so the sinks saturate mid-game instead of by ~run 9; report + targets in `design/economy-sim.md`. Anchors held: cheapest building L1 = 40 gold, shard value 5, tech-node costs.)*
- **Town-produced (day tick):** Knowledge (Study, Library→University, Observatory), Stone (Quarry — the only Age-I material; Timber cut 2026-07-10), Food (Farm — **upkeep → Well-Fed bonus**: town consumes Food per tick; covered = +% to all other production incl. Knowledge, short = no bonus, never a penalty; the upkeep mechanism is the Act-II army/city prepare-for — `food-upkeep.md`, designed 2026-07-03), Gold (Market).
...
- **Risk:** dumping multiple story scenes after one run (mitigated by 1-forced-per-visit rule); content volume (agent-drafted against `voice-guides.md`, human-curated).
...
## 9. UX & Feedback
- **Must be visible:** current HP / resources / active Echoes (in-run HUD). **The in-run HUD is built (2026-07-07; rebuilt into the "Ember" language 2026-08-10, `design/ui-hud.md`):** a top-right resource readout + room block (header `F2 · R3/5 ⚠ · Wave 2/3` over objective rows), a bottom-left echo rail of monogram medallions growing upward, a bottom-centre HP bar with a low-HP edge vignette, bottom-right ability rings (RMB/Q/R + dash) with cooldown arcs, and a top-centre boss bar. **There is no on-screen instruction text** — the contextual hint line was removed on a human directive, 2026-08-14 (`ui-hud.md` § "No on-screen instructions"); the world carries the affordance. Room/floor progress + minimap (Cartography tech reveals more — incl. door foresight, `run-structure.md`); **door reward sigils** (+ peril marks; first encounter of each sigil gets a one-line tooltip); active tech node + progress; building costs vs. owned resources; which NPC has new dialogue (`!`/`!!` marker, `!!` = a forced/spine story beat).
- **Must be immediate:** hit/dash/pickup feedback (the feel-critical SFX subset gets human tuning); ability cooldown states (RunHud slot darken + seconds); Echo-pick HUD (auto-opens).
...
- **Caps / soft caps:** Food upkeep is bonus-only (Well-Fed or not — no starvation state, no spiral; `food-upkeep.md`); buildings cap at L3 *within Age I* (age-banded levels: +2 per later age, cap L11 — `town-economy.md`); attunements/etchings cap at 3 levels; weapons flat at 5 / resonance at 4 effects; assist-mode resist capped.
...
EventBus + Ledger; save/slots/migration + profile; the unlock cascade; combat (3 weapons, 9 etchings, ~7 attunements, dash); Echoes (~50); dungeon gen (5 floors as strata: profiles + 5 signature hazards + 1 shared); 12 enemies + 5 bosses + elite modifiers; tech tree (14 nodes, 8 bespoke + 6 light puzzles, auto-solve); town (9 buildable + 2 shops, age-banded levels — Age I = 3; `town-economy.md`) + day tick; resource economy (Medieval roles + retirement scaffolding); codex assembly; dialogue system (~358 pieces total) + cutscenes; achievements (~25); assist mode + auto-solve; 2.5D rendering + painterly static screens.
