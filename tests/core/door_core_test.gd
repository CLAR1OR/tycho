...
func test_cache_reward_scaling_and_peril() -> void:
	...
	check_eq(g1["amount"], 6.0, "floor-1 gold cache = base")
	var g3 := DoorCore.cache_reward("gold", 3, false)
	check_eq(g3["amount"], 12.0, "floor-3 gold cache = base + 2*per-floor")
	var g3p := DoorCore.cache_reward("gold", 3, true)
	check_eq(g3p["amount"], 24.0, "peril doubles the cache")
